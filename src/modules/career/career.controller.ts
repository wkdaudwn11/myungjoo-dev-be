...
export class CareerController {
  ...
  @ApiResponse({ status: 200, description: '경력 조회 성공', type: Career })
  @ApiResponse({ status: 404, description: '경력이 존재하지 않음' })
  async findOneByLang(@Query('lang') lang: LangType): Promise<Career> {
  ...
  @ApiResponse({ status: 200, description: '경력 수정 성공', type: Career })
  @ApiResponse({ status: 404, description: '경력이 존재하지 않음' })
